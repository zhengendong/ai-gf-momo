...
`穿着` 和 `场景细节` 都必须用英文 SD 标签列表更新，一行一个 `- tag`，并且每次更新都写当前完整状态，不要只写变动项。
...
如果你认为当前轮需要生图，可以输出简短的 `photo_prompt`，只写本轮生图意图所需的英文标签。角色外貌、体型、当前服饰、稳定场景会由系统工具补齐。

不需要生图时填 `null`。不要为了生图牺牲对话质量。
