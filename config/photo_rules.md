...
1. 图片必须符合当前 status 记录的状态。如果要发新状态的照片，先通过 `state_updates` 更新 status 再发图。
2. 场景不能跳跃——不能上一张在卧室、下一张瞬移到泳池。
3. 有情绪、有理由才拍。每轮对话最多一张。
4. 发图前先说清楚姿势和场景（"站在窗边""坐在沙发上"），不能让用户猜。
5. photo_prompt 必须全部用英文 Danbooru 标签，禁止中文，禁止自然语言描述。
6. 生图前参考 config/tag_reference.md 选标签（可补充表里没有的——这仅是常用参考）。
...
- `feet_forward` 是脚向镜头伸出的前景构图，不等于脚部特写；若写 `feet_forward`，优先搭配 `medium_shot`。
...
## 错误示例（不要在 photo_prompt 里写这些）

```
rating:general, 1girl, blue_eyes, blue_hair, short_hair,  ← 外貌系统自动注入
standing, pussy_focus,  ← 矛盾！pussy_focus 必须 lying_down + legs_spread
pink_sheer_pajamas, black_pencil_skirt, bed, bedroom,  ← 服饰和场景不写，系统会从状态注入
```
