...
只有 `image_goal` 存在时 `shot_spec` 才是对象。用户明确要求观看、展示或拍照而角色回复已经接受并实际呈现时，MomoAgent 必须设置该目标，不能因状态未变化而跳过。VisualContinuityAgent 是最终提示词总编辑：它接收完整 `role/body/appearance` 候选和服饰、场景状态，按原顺序完整输出 `role_tags/body_tags/appearance_tags`，并规划动作、姿势、表情、景别、角度、焦点、光线与 rating。人物三组锚点和当前实际服饰/裸露事实都不是可选创作项；后端会回填缺失或旧格式字段，避免局部特写让角色凭空恢复衣物。场景标签仍由导演按画面选择。它还可用彼此分离的 `action_text/environment_text` 英文短语补充标签难以表达的人物交互、道具关系和空间关系。质量词、负面词、工作流和模型仍由系统统一管理。

状态事务与图片编辑错误分属不同失败边界。VisualContinuityAgent 的一次调用可以在协议错误后自行修复一次，但运行时不得为 state-only 或提示词兜底追加新的模型调用。镜头冲突、缺失 ShotSpec、提示词长度或自然语言格式应在导演的同次输出解析中降级处理；若两次输出仍无法可靠提交状态，则保持上轮冻结快照，存在图片目标时以此构造本地基础镜头继续生图，不猜测未经验证的状态变化。

VisualContinuityAgent 会在固定协议后附加 `config/knowledge/visual_prompting.md`。该手册从 `data/pxlsan-标签选择器-完整内容.xlsx` 的服装、动作、构图、场景和光影栏目蒸馏而来，只保留组合规律、代表性词汇、few-shot 和冲突规则，不把原表数千行标签塞入每轮上下文。存在图片目标时，导演必须先确定一个可见的核心动作，再以必要姿势和镜头使它成立；精确标签不能表达的手部、身体、对象和道具关系由 `action_text` 补充。构图中的景别与部位焦点二选一，角度可独立保留。人物 `role/body/appearance` 全部保留，环境道具和空间关系可由独立 `environment_text` 补充。最终提示词不设标签数或自然语言长度硬上限，长度永远不能导致图片或整轮对话被取消。所有数值权重均已退出协议，局部特写通过删除无关服饰和场景候选建立重点。
...
`ImageJob` 冻结角色、本轮回复、`image_goal`、`shot_spec`、服饰、场景和状态版本。正常运行时的 `build_image_prompt()` 只按固定优先级序列化导演已审核的混合计划：质量与 rating → 完整人物 `role/body/appearance` 标签 → 完整当前服饰与裸露事实 → 构图视角 → 姿势、动作标签与动作短语 → 精选环境标签、环境短语与光线。约 40 个语义单元仅是导演面向 SDXL 的编辑目标，不是后端计数、截断或拒绝规则；后端只本地压缩超长动作/环境短语，不再编译任何权重。第一阶段运行时会复用本轮已读取的聊天历史和最终 Momo 提示词，图片任务优先于低优先级持久化任务。
